...
def to_rref(M):
    ...
    """
    # --- YOUR CODE HERE ---
    # Implement the Gauss-Jordan elimination algorithm to find the RREF.
    
    # Suggested Steps:
    # 1. Make a copy of the matrix to avoid modifying the original.
    # 2. Initialize pivot row and pivot column indices.
    # 3. Loop through the matrix to perform forward elimination:
    #    a. If the current pivot element is zero, swap with a row below it
    #       that has a non-zero element in the same column.
    #    b. Normalize the pivot row so the pivot element is 1.
    #    c. Eliminate other non-zero entries in the pivot column by
    #       subtracting multiples of the pivot row.
    # 4. After the forward pass, perform backward elimination to create
    #    zeros above the pivots.
    
    # Placeholder: Return the original matrix until implemented
...
def find_nullspace_basis(A):
    ...
    # --- YOUR CODE HERE ---
    # Use the RREF of A to find the basis for the nullspace.
    
    # Suggested Steps:
    # 1. Identify the pivot columns and free columns. A pivot column contains
    #    the first non-zero entry (a '1') of a row. Other columns are free.
    # 2. For each free column, create a "special solution":
    #    a. Create a vector of zeros of size n (number of columns in A).
    #    b. Set the component corresponding to the current free variable to 1.
    #    c. Set components for other free variables to 0.
    #    d. Solve for the pivot variables using the equations from the RREF.
    #       The value of a pivot variable will be the negative of the entry
    #       in the RREF in that pivot row and the current free column.
    # 3. Append each "special solution" vector to the basis_vectors list.
...
def find_particular_solution(A, b):
    ...
    # --- YOUR CODE HERE ---
    # Use the RREF to find one particular solution.
    
    # Suggested Steps:
    # 1. Check for inconsistency. If any row is of the form [0, 0, ..., 0 | c]
    #    where c is non-zero, then no solution exists. Return None.
    # 2. Identify pivot and free columns (same as in the nullspace function).
    # 3. To find a *particular* solution, set all free variables to 0.
    # 4. Create a solution vector of zeros of size n.
    # 5. For each pivot variable, its value is given by the last column
    #    of the RREF in the corresponding pivot row.
    # 6. Fill these values into your solution vector.
